...
function specialize_method(match::OverdubMethodMatch, preexisting::Bool=false, compilesig::Bool=false)
    shadow = specialize_method(unwrap_shadow(match), preexisting, compilesig)::MethodInstance
    if !haskey(OVERDUB_CACHE_TABLE, shadow)
        overdub = specialize_method(unwrap_overdub(match), preexisting, compilesig)::MethodInstance
...
function retrieve_code_info_overdubbed(shadow::MethodInstance)
    ...
    return transform_overdub_src!(overdub, copy(src))
end

"""
    transform_overdub_src!(overdub::MethodInstance, src::CodeInfo)

Transform `src` the code of a overdubbed method so that it can fit into the `SHADOW_METHOD`'s call signature:
1. prepend SSA statements so that they can be referred instead of the overdubbed call arguments
...
5. modify all the `:call`s so that it will be overdubbed again by context, which is propagated as the first argument

Note that the step 4. modifies all the `:call`s whatever it can be overdubbed or not.
`fixup_no_overdubs!` will re-transform those calls that can't be overdubbed after the inference.
"""
function transform_overdub_src!(overdub::MethodInstance, src::CodeInfo)
    # fixup slot inforamtion
